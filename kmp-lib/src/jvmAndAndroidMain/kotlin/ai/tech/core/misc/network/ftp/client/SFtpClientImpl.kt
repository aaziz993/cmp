...
public open class SFtpClient(
    ...
    override fun connect() {
        with(config) {
            FtpClientConfig.hostKeyVerifierFingerprint?.let { client.addHostKeyVerifier(it) }
            with(host) {
    ...
    override fun login() {
        with(config) {
            with(host) {
                if (FtpClientConfig.privateKey == null) {
                    client.authPassword(username, password)
                }
                else {
                    val kp = client.loadKeys(FtpClientConfig.privateKey, password)
