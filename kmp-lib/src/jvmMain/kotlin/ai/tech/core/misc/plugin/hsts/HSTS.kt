...
public fun Application.configureHSTS(config: HSTSConfig?, block: (io.ktor.server.plugins.hsts.HSTSConfig.() -> Unit)? = null) {
    val configBlock: (io.ktor.server.plugins.hsts.HSTSConfig.() -> Unit)? = config?.takeIf(EnabledConfig::enabled)?.let {
        {
            it.global?.takeIf(EnabledConfig::enabled)?.let {
                it.preload?.let { }
            ...
            it.hostSpecific?.filterValues(EnabledConfig::enabled)?.forEach {
                withHost(it.key) {
